...
async def _run_local_rag_singleflight(
    ...
    future.add_done_callback(_release_local_rag_slot)
...
def common_tts_prewarm_items() -> list[tuple[str, str]]:
    items: list[tuple[str, str]] = [
        ...
        ("Sorry, I couldn't find a reliable answer in my knowledge base. Please visit A I F C dot K Z.", "en"),
        ...
        ("Извините, я не нашел надежный ответ в своей базе знаний. Пожалуйста, посетите A I F C точка K Z.", "ru"),
        ...
        ("Кешіріңіз, мен білім базасынан сенімді жауап таба алмадым. A I F C нүкте K Z сайтына кіріңіз.", "kk"),
        ("您好，欢迎咨询。我可以帮助解答 AIFC 服务、注册、监管、法院、仲裁和金融科技相关问题。", "zh"),
        ("再见。如果之后您有 AIFC 相关问题，我可以继续帮助您。", "zh"),
        ("抱歉，我没有在知识库中找到可靠答案。请访问 A I F C 点 K Z。", "zh"),
    ]
...
async def _gemini_local_rag_candidate(
    ...
    *,
    ...
    on_spoken_delta: GeminiSpokenCallback | None = None,
    ...
    raw = ""
    emitted_spoken_chars = 0

    async def maybe_emit_spoken_delta() -> None:
        nonlocal emitted_spoken_chars
        if on_spoken_delta is None:
            return
        if stream_gate is not None and not stream_gate.is_set():
            return
        update = _spoken_delta_from_partial_json(raw, language, emitted_spoken_chars)
        if update is None:
            return
        delta, emitted_chars, complete = update
        await on_spoken_delta(delta, emitted_chars, complete)
        emitted_spoken_chars = emitted_chars

    async for delta in stream_answer(history_msgs, prompt):
        raw += delta or ""
        await maybe_emit_spoken_delta()
    await maybe_emit_spoken_delta()
    if not raw.strip():
        return None
...
async def run_answer_race(
    ...
    conversation_memory: dict | None,
    *,
    on_gemini_spoken_delta: GeminiSpokenCallback | None = None,
    ...
    local_task = asyncio.create_task(_timed("local_rag", _local_rag_candidate(query, language, history, conversation_memory)))
    gemini_stream_gate = asyncio.Event() if on_gemini_spoken_delta is not None else None
    gemini_task = asyncio.create_task(
        _timed(
            ...
            _gemini_local_rag_candidate(
                ...
                conversation_memory,
                local_task,
                stream_gate=gemini_stream_gate,
                on_spoken_delta=on_gemini_spoken_delta,
    ...
    tasks: set[asyncio.Task[RaceCandidate | None]] = {
        ...
        asyncio.create_task(_timed("answer_cache", _memory_cache_candidate(query, language))),
        asyncio.create_task(_timed("faq", _faq_candidate(query, language))),
        asyncio.create_task(_timed("cache", _cache_candidate(query, language))),
        gemini_task,
    }
    ...
    if winner is None:
        if gemini_stream_gate is not None:
            gemini_stream_gate.set()
    ...
    if not local_task.done():
        local_task.cancel()
    for task in tasks:
        if task is not gemini_task or winner is not None:
            task.cancel()
    await asyncio.gather(*tasks, local_task, return_exceptions=True)
