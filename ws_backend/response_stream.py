...
class ResponseStream:
    ...
    def __init__(
        ...
        *,
        ...
        self._media_queue: asyncio.Queue[tuple[str, int, str | None] | None] | None = (
            asyncio.Queue() if self._streaming_media else None
        )
        self._avatar_queue: asyncio.Queue[tuple[int, bytes] | None] | None = (
            asyncio.Queue() if self._streaming_media else None
        )
        self._media_worker_task: asyncio.Task | None = None
        self._avatar_worker_task: asyncio.Task | None = None
    ...
    def _schedule_spoken_chunk(self, sentence: str, idx: int, lang: str | None) -> None:
        ...
        if not cleaned:
            return
        if self._streaming_media and self._media_queue is not None:
            self._ensure_media_worker()
            self._media_queue.put_nowait((cleaned, idx, lang))
            return

        self._chunk_count = max(self._chunk_count, idx + 1)
        task = asyncio.create_task(self._run_limited_chunk(cleaned, idx, lang))
    ...
    def _ensure_media_worker(self) -> None:
        ...
        if self._avatar_worker_task is None or self._avatar_worker_task.done():
            self._avatar_worker_task = asyncio.create_task(self._run_streaming_avatar_worker())
            self._chunk_tasks.append(self._avatar_worker_task)
    ...
    async def _run_streaming_media_worker(self) -> None:
        ...
        try:
            while True:
                ...
                if item is None:
                    return
                sentence, text_idx, lang = item
                await self._run_streaming_sentence(sentence, text_idx, lang)
        finally:
    ...
    async def _run_streaming_sentence(self, sentence: str, text_idx: int, lang: str | None) -> None:
        ...
        target_bytes = _segment_target_bytes(sample_rate, SONIOX_TTS_SEGMENT_MS)
        min_tail_bytes = _pcm_bytes_for_ms(sample_rate, SONIOX_TTS_MIN_SEGMENT_MS)
        buffer = bytearray()

        try:
            async for pcm in self._tts.synthesize_pcm_stream(sentence, lang=lang):
                if not pcm:
                    continue
                buffer.extend(pcm)
                while True:
                    segment_target = (
                        _segment_target_bytes(sample_rate, SONIOX_TTS_FIRST_SEGMENT_MS)
                        if self._media_chunk_idx == 0
                        else target_bytes
                    )
                    if len(buffer) < segment_target:
                        break
                    segment = bytes(buffer[:segment_target])
                    del buffer[:segment_target]
                    await self._queue_pcm_segment(segment, sample_rate, text_idx)
            if buffer:
                await self._queue_pcm_segment(_pad_pcm_tail(bytes(buffer), min_tail_bytes), sample_rate, text_idx)
        except ClientClosedError:
            raise
        except asyncio.CancelledError:
            raise
        except Exception:
            log.exception("streaming TTS/avatar segment failed: text_chunk=%s", text_idx)
    ...
    async def _run_limited_chunk(self, sentence: str, idx: int, lang: str | None) -> None:
        try:
            audio_wav = await self._tts.synthesize(sentence, lang=lang, priority=0 if idx == 0 else 1)
            audio_b64 = base64.b64encode(audio_wav).decode("ascii")
            await self._writer.send(
                self._event(
                    {
            ...
            async for frame in self._synctalk.infer_stream(audio_wav, priority=0 if idx == 0 else 1, chunk_idx=idx):
                await self._writer.send(self._event({"type": "frame", "data": frame, "chunk": idx}))
        except ClientClosedError:
            raise
        except asyncio.CancelledError:
            raise
            ...
            try:
                await self._writer.send(self._event({"type": "chunk_done", "chunk": idx}))
            except ClientClosedError:
                pass
    ...
    async def wait_all(self) -> None:
        if self._streaming_media and self._media_queue is not None and not self._media_closed:
