...
class LowLatencyVoiceChunker:
    ...
    It emits TTS chunks at a period boundary or after a smooth spoken phrase.
    The word limit is based on human words, not model tokens or characters.
    """
    ...
    def _next_cut(self, final: bool) -> int | None:
        ...
        candidates = [cut for cut in (sentence_cut, threshold_cut, word_count_cut, max_cut) if cut is not None]
        if candidates:
